...
| 标签 | 预计时长 | 阅读范围 | 最小完成动作 |
| --- | ---: | --- | --- |
| **必读** | 30–45 分钟 | [Document AI 定义](#part-1-what-is-document-ai)、[演进路线](#part-2-evolution-of-document-ai)，再选择 Docling、MinerU 或 OmniDocBench 一个节点 | 在一页样例上标出 5 类任务，并确定当前主指标 |
| **选读** | 1–2 小时 | 按项目选择 LayoutLM 系列、现代解析工具、VLM 或结构化表示章节 | 画出“输入—模型/工具—输出—指标”流程 |
| **深挖** | 半天至 1 周 | 阅读所选工具/模型的官方论文、仓库与评测协议 | 跑通一个公开样例并保存版本、原始输出和失败案例 |

!!! info "本页学习卡"
    **学什么**：OCR、版面、结构、问答和端到端解析的边界与演进。<br>
    **官方来源**：模型论文、Docling/MinerU 官方仓库和 Benchmark 官方资料。<br>
    **做什么**：把当前项目映射到一个明确任务和输出。<br>
    **提交什么**：任务标注、pipeline 图、主指标与一个失败样例。<br>
    **如何自查**：能说明模型错误、格式错误与评测对齐错误之间的区别。
