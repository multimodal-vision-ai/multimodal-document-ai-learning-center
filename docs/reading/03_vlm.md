...
| 标签 | 预计时长 | 阅读范围 | 最小完成动作 |
| --- | ---: | --- | --- |
| **必读** | 30–45 分钟 | [VLM 基本结构](#part-1-what-is-a-vision-language-model)，再从 LLaVA、Florence-2 或 Qwen 系列选择一个模型 | 画出一个 VLM 流程，并记录一个能力和一个边界 |
| **选读** | 1–2 小时 | 按任务选择开源模型演进、下一代 VLM 或实验设计章节 | 完成模型选择表：任务、规模、输入、输出、限制 |
| **深挖** | 半天至 1 周 | 阅读所选模型的 Model Card、论文、官方代码和评测 | 用固定样例完成一次受控实验并保存失败案例 |

!!! info "本页学习卡"
    **学什么**：视觉编码、跨模态连接、语言生成与常见失败。<br>
    **官方来源**：模型正式论文、Model Card 与官方代码仓库。<br>
    **做什么**：选择一个与当前任务和算力匹配的模型进行验证。<br>
    **提交什么**：结构图、模型选择理由、成功与失败样例。<br>
    **如何自查**：能区分模型宣称能力、公开评测结果和自己的实验结论。
...
[上一章](02_document-ai.md){ .md-button }    [下一章](04_doctags.md){ .md-button }
