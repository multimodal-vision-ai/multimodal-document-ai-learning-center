...
| 标签 | 预计时长 | 阅读范围 | 最小完成动作 |
| --- | ---: | --- | --- |
| **必读** | 30–45 分钟 | [按任务选择](#task-selector)，再进入一个最匹配的 Benchmark | 写出研究问题、数据集、主指标和失败案例格式 |
| **选读** | 1–2 小时 | 阅读所选 Benchmark 的官方资源、任务、指标、数据来源与限制 | 完成一页 Benchmark Card，并判断是否适配当前数据 |
| **深挖** | 半天至 1 周 | 阅读选择策略和评测流程，运行一个固定小切片 | 保存评测版本、原始结果、成本和错误 taxonomy |

!!! info "本页学习卡"
    **学什么**：怎样让数据、指标和失败案例共同回答研究问题。<br>
    **官方来源**：Benchmark 论文、官方仓库、数据集主页与评测脚本。<br>
    **做什么**：先选能力，再选一个数据集和一个主指标。<br>
    **提交什么**：Benchmark Card、评测配置、原始结果和错误分析。<br>
    **如何自查**：指标与任务匹配，数据许可清楚，结论没有被总体分数掩盖。
