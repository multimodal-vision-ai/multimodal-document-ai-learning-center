...
这里不是零散代码示例，而是一条可以自己推进的 **project lab**。你会从一个明确问题出发，把项目组织、数据准备、模型推理、受控实验、评测和研究报告串成一个可复现作品。
...
你不需要一次理解全部内容。只要每一关都留下可运行文件、原始结果或研究记录，七关结束时，它们会自然组成最终项目。

!!! tip "卡住时的处理顺序"
    先回到本关的官方入口核对版本与输入格式；再缩小为 1 个样例和 1 次运行；最后把错误信息、已经尝试的方法和当前环境写入实验日志。不要用手工修改输出的方式制造“成功结果”。

## 当前可做的项目

| 项目 | 状态 | 适合周次 | 建议投入 | 核心成果 |
| --- | --- | --- | ---: | --- |
| [项目 01｜Qwen3.5-0.8B 文档理解与评测](Experiment_01/README.md) | 可开始 | Week 4–7 | 12–18 小时 | 推理 pipeline、Prompt 对照、小规模评测与项目报告 |

目前只有项目 01 达到可学习状态。未完成的方向不会显示为“可用实验”，避免学生点进空入口。

## 项目 01｜Qwen3.5-0.8B 文档理解与评测
...
| 阶段 | 你要完成什么 | 主要产出 |
| --- | --- | --- |
| [Part 01](Experiment_01/Part01-实验准备.md) | 确定研究问题与完成标准 | 实验计划与检查清单 |
...
| [Part 07](Experiment_01/Part07-Bechmark评测与总结.md) | 评测并形成结论 | 指标表、错误分析与总结报告 |
...
| 不只是完成…… | 更重要的是学会…… |
| --- | --- |
| 跑通一个模型 | 把模型版本、输入、配置与输出组成证据链 |
| 尝试几个 Prompt | 只改变一个变量，做能够解释的对照实验 |
| 得到一个分数 | 判断指标是否回答了原来的问题 |
| 写一份 README | 让另一位同学能够复现并质疑你的结论 |
